...
public static class SailParser
{
    ...
    private static SailIngressOptions HandleAnnotations(SailIngressContext context, V1ObjectMeta metadata)
    {
        ...
        if (annotations.TryGetValue("sail.ingress.kubernetes.io/backend-protocol", out var http))
        {
            options.Https = http.Equals("https", StringComparison.OrdinalIgnoreCase);
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/transforms", out var transforms))
        {

            options.Transforms = YamlDeserializer.Deserialize<List<Dictionary<string, string>>>(transforms);
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/authorization-policy", out var authorizationPolicy))
        {
            options.AuthorizationPolicy = authorizationPolicy;
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/cors-policy", out var corsPolicy))
        {
            options.CorsPolicy = corsPolicy;
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/session-affinity", out var sessionAffinity))
        {
            options.SessionAffinity = YamlDeserializer.Deserialize<SessionAffinityConfig>(sessionAffinity);
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/load-balancing", out var loadBalancing))
        {
            options.LoadBalancingPolicy = loadBalancing;
        }

        if (annotations.TryGetValue("sail.ingress.kubernetes.io/http-client", out var httpClientConfig))
        {
